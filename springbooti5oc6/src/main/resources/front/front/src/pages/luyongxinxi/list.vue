...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 0","alignItems":"center","background":"none","display":"flex","width":"100%","justifyContent":"flex-end","height":"auto"}'>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>求职者姓名</div>
        <el-input v-model="formSearch.qiuzhizhexingming" placeholder="求职者姓名" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	  ...
	  <el-button v-if="isAuth('luyongxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#1e8892","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/luyongxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"padding":"120px 7% 100px","margin":"20px 0 10px","background":"url(http://codegen.caihongy.cn/20230109/138a4d606eae4a168810cef229969885.png) no-repeat center top,url(http://codegen.caihongy.cn/20230109/b95aeae1c1294f398aefb7c6cd5b16de.png) no-repeat center top,url(http://codegen.caihongy.cn/20230109/5ffc803e6682418eb7f0b09a98e35527.png) no-repeat center bottom,#faf0e6"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"1px solid #ddd","padding":"10px","margin":"0 0 20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-if="item.luyongtupian && item.luyongtupian.substr(0,4)=='http'" :src="item.luyongtupian" class="image" />
				<img :style='{"width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.luyongtupian?item.luyongtupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"border":"1px solid #eee","padding":"4px 10px","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"20px","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.qiuzhizhexingming}}</div>
				<div :style='{"border":"1px solid #eee","padding":"4px 10px","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"20px","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.zhiweimingcheng}}</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.qiuzhizhexingming != '') searchWhere.qiuzhizhexingming = '%' + this.formSearch.qiuzhizhexingming + '%';
        if (this.formSearch.zhiweimingcheng != '') searchWhere.zhiweimingcheng = '%' + this.formSearch.zhiweimingcheng + '%';
        this.$http.get('luyongxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/luyongxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
