	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="求职日期" prop="qiuzhiriqi">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.qiuzhiriqi" label="求职日期" prop="qiuzhiriqi">
					<el-input v-model="ruleForm.qiuzhiriqi" placeholder="求职日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="求职岗位" prop="qiuzhigangwei">
					<el-input v-model="ruleForm.qiuzhigangwei" placeholder="求职岗位" clearable  :readonly="ro.qiuzhigangwei"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="求职岗位" prop="qiuzhigangwei">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="教育背景" prop="jiaoyubeijing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.jiaoyubeijing" label="教育背景" prop="jiaoyubeijing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.jiaoyubeijing"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					var json = data.data;
					if(((json.qiuzhizhezhanghao!=''&&json.qiuzhizhezhanghao) || json.qiuzhizhezhanghao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.qiuzhizhezhanghao = json.qiuzhizhezhanghao
						this.ro.qiuzhizhezhanghao = true;
					}
					if(((json.qiuzhizhexingming!=''&&json.qiuzhizhexingming) || json.qiuzhizhexingming==0) && this.$storage.get("role")!="管理员"){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.huojiangzhengshu = this.ruleForm.huojiangzhengshu.replace(reg,'../../../springbooti5oc6/upload');
        this.ruleForm.jiaoyubeijing = this.ruleForm.jiaoyubeijing.replace(reg,'../../../springbooti5oc6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
